/**
 * Teacher - Detail Kegiatan Page
 * Displays schedule detail in read-only mode with back button
 * Only shows schedules for teacher's class
 */
...
import { getScheduleById } from '@/actions/teacher/schedule'
...
export default function ScheduleDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const loadSchedule = async () => {
        setLoading(true)
        try {
            const result = await getScheduleById(Number(id))
            if (result.success && result.data) {
                setSchedule(result.data)
            } else {
                alert('Jadwal tidak ditemukan')
                router.push('/teacher/jadwal')
            }
        } catch (error) {
        ...
        <DashboardLayout role="Pembina" menuItems={pembinaMenuItems}>
            <div className="p-6 md:p-8">
                ...
                <div className="flex justify-between items-center mb-8">
                    ...
                    <button
                        onClick={() => router.push('/teacher/jadwal')}
